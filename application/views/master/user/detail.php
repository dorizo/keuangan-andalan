...
<div class="col-lg-12">
        <div class="card card-primary">
          <div class="card-header">
            ...
            <div class="card-body">
            <form action="<?php echo base_url() ?>Master/user/add_detail" method="post" role="form" enctype="multipart/form-data">
              
                <div class="form-group">
                  <label>email <?= $user->email ?></label>
                  <input type="hidden" name="userCode" value="<?= $user->userCode ?>">
                  <select class="permission select2bs4" multiple="multiple" data-placeholder="Select a role" name="roleCode" style="width: 100%;" required="required">
                    <?php
                    if(!empty($permission)){
                      foreach($permission as $value){
                        echo "<option value='$value->roleCode' select>$value->role</option>";
                      }}
                      ?>
                  </select>
        ...
        <div class="card-body" >
         ...
         <a href="<?php echo base_url() ?>Master/user/delete_detail/<?= $value->ruCode; ?>">
          <button type="button" class="btn btn-secondary mb-1" >
            <i class="fas fa-times-circle"></i> <?= $value->role ?>  
          </button>
          </a>
          <?php } ?>
