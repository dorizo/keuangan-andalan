        ...
        if (is_array($this->input->get("witel_id")) || is_object($this->input->get("witel_id")))
        {
            foreach ($this->input->get("witel_id") as $key => $value) {
                ...
                for ($bulan=0; $bulan < 12 ; $bulan++) { 
                    # code...
                    $querrrrrV = 'SELECT sum((nilai_boq-nilai_project_paid)) as boq from project where witel_id="'.$vendor->witel_id.'" AND DATE_FORMAT(project_date , "%m") = '.($bulan+1).' AND status_paid="OUTSTENDING"  AND DATE_FORMAT(project_date , "%Y") = "'.$tahunss.'" GROUP BY DATE_FORMAT(project_date , "%Y-%m") ASC;';
                    // echo $querrrrrV ;
