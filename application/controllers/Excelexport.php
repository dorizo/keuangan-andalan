...
class Excelexport extends CI_Controller {
	...
	public function transaksi()
	{
		...
		if($this->input->get()){
			$data["dataresult"] = $this->akunbankTransaksi_model->export($this->input->get());
	
		}else{
			$data["dataresult"] = $this->akunbankTransaksi_model->export(array());
		...
		$data["titlepage"] = "REPORT TRANSAKSI PROJECT";
		$this->load->view('template/header' , $data);
		$this->load->view('excel/transaksi' , $data);
		$this->load->view('template/footer');
	}
	...
	public function exporttransaksi(){
		...
		if($this->input->get()){
			$dataresult = $this->akunbankTransaksi_model->export($this->input->get());
	
		}else{
			$dataresult = $this->akunbankTransaksi_model->export(array());
		}
		...
		$sheet->setCellValue('A1', "APPROVEL DATE"); // Set kolom A3 dengan tulisan "NO"
		$sheet->setCellValue('B1', "AKUN TRANSAKSI"); // Set kolom B3 dengan tulisan "NIS"
		$sheet->setCellValue('C1', "WITEL"); // Set kolom C3 dengan tulisan "NAMA"
		$sheet->setCellValue('D1', "CATEGORY"); // Set kolom D3 dengan tulisan "JENIS KELAMIN"
		$sheet->setCellValue('E1', "PROJECT CODE"); // Set kolom E3 dengan tulisan "ALAMAT"
	
		$sheet->setCellValue('F1', "DESCRIPTION"); // Set kolom A3 dengan tulisan "NO"
		$sheet->setCellValue('G1', "AMAUNT"); // Set kolom B3 dengan tulisan "NIS"
		$sheet->setCellValue('H1', "AKUN BANK"); // Set kolom C3 dengan tulisan "NAMA"
        ...
        foreach ($dataresult as $key => $value) {
		  ...
		  $sheet->setCellValue('A'.$numrow, $value["transaksiDate"] );
		  $sheet->setCellValueExplicit('B'.$numrow, $value["akunbankName"],  \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING );
		  $sheet->setCellValue('C'.$numrow, $value['witel_name']);
		  $sheet->setCellValue('D'.$numrow,  $value["cat_name"]);
		  $sheet->setCellValue('E'.$numrow, $value["project_code"]);
		  $sheet->setCellValue('F'.$numrow, $value["transaksiNote"]);
		  $sheet->setCellValue('G'.$numrow, $value["transaksiJumlah"]);
		  $sheet->setCellValue('H'.$numrow, $value["bank_name"]);
		...
		$sheet->getColumnDimension('F')->setWidth(50); // Set width kolom A
		...
		$sheet->setTitle("Laporan Data Transaksi Api");
	
		// Proses file excel
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="REPORT_TRANSAKSIAPI.xlsx"'); // Set nama file excel nya
